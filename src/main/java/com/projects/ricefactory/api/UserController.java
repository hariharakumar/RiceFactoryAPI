    ...
    public ResponseEntity updateUser(@RequestBody User updatedUser, @PathVariable("id") Long userId) {
        ...
        try {
            ...
            if(existingUser == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("User record you are trying to update doesn't exist");
            }

            User updatedUserObject = userServiceDao.updateUser(existingUser,updatedUser);

            return new ResponseEntity<User>(updatedUserObject, HttpStatus.OK);
